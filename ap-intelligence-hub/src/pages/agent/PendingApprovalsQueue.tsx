...
const QUEUE_STATUSES: CaseStatus[] = ['APPROVAL_PENDING'];
...
export function PendingApprovalsQueue() {
  ...
  const filteredCases = cases.filter((c) => c.status === 'APPROVAL_PENDING');
  ...
  return (
    <div>
      <PageHeader title="Pending Approvals" count={filteredCases.length}>
      ...
      <p className="text-sm text-muted-foreground -mt-2 mb-4">Track cases currently moving through the approval chain and awaiting sign-off.</p>
      <FilterBar
        filters={filters}
      ...
      {isLoadingCases ? (
        ...
        <EmptyState
          ...
          description="No cases are currently in the approval chain."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filteredCases.map((c) => (
            <CaseCard
              ...
              caseData={c}
              variant="pending"
              onClick={(id) => navigate(`/agent/cases/${id}/approval`)}
            />
          ))}
        </div>
      ) : (
            ...
            <TableBody>
              {filteredCases.map((c) => (
                <TableRow
                  ...
                  onClick={() => navigate(`/agent/cases/${c.id}/approval`)}
                >
                  ...
                  <TableCell className="text-muted-foreground text-sm py-3">
                    {formatRelativeTime(c.approvalChain?.createdAt || c.createdAt)}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
